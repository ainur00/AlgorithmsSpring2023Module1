...
std::vector<Complex> FFTRecursion(const std::vector<Complex>& inputData) {
    if (inputData.size() == 1)
        return inputData;
    std::vector<Complex> oneRRoots = GetRootsFromOne(inputData.size(), true);
    ...
    splittedDataFFT[0] = FFTRecursion(splittedData[0]);
    splittedDataFFT[1] = FFTRecursion(splittedData[1]);
    for (uint32_t i = 0; i < inputDataFFT.size(); ++i)
        inputDataFFT[i % inputDataFFT.size()] = splittedDataFFT[0][i % splittedDataFFT[0].size()] + splittedDataFFT[1][i % splittedDataFFT[1].size()] * oneRRoots[i % oneRRoots.size()];
    return inputDataFFT;
}
...
std::vector<Complex> FFT(const int16_t* inputData, uint32_t inputDataSize) {
    ...
    return FFTRecursion(inputDataVector);
}

std::vector<Complex> Modify(const std::vector<Complex>& fftData) {
    std::vector<Complex> modifiedData = fftData;
    for (uint32_t i = 0; i < modifiedData.size() / 4; ++i)
        modifiedData[i] = Complex(0.0, 0.0);
    for (uint32_t i = 3 * modifiedData.size() / 4; i < modifiedData.size(); ++i)
        modifiedData[i] = Complex(0.0, 0.0);
    return modifiedData;
}

std::vector<Complex> IFFTRecursion(const std::vector<Complex>& modifiedData) {
    ...
    splittedDataIFFT[0] = IFFTRecursion(splittedData[0]);
    splittedDataIFFT[1] = IFFTRecursion(splittedData[1]);
    for (uint32_t i = 0; i < modifiedDataIFFT.size(); ++i)
        modifiedDataIFFT[i % modifiedData.size()] = (splittedDataIFFT[0][i % splittedDataIFFT[0].size()] / modifiedDataIFFT.size() + splittedDataIFFT[1][i % splittedDataIFFT[1].size()] / modifiedDataIFFT.size() * oneRoots[i % oneRoots.size()]) * modifiedDataIFFT.size();
...
int16_t* IFFT(const std::vector<Complex>& modifiedData) {
    ...
    std::vector<Complex> IFFTModifiedData = IFFTRecursion(modifiedData);
    for (uint32_t i = 0; i < modifiedData.size(); ++i)
